...
def custom_grid_canvas(master, grid, size_cell, margin=10, gutter=5, show_vals=True, outline=True):
    ...
    return Canvas(master, width=x_cnv*size_cell+2*margin + gutter*(x_cnv-1), height=y_cnv*size_cell+2*margin +gutter*(y_cnv-1) , bg ='white')
...
def grid_canvas(master, grid, size_cell, margin=10, gutter=5, show_vals=True, outline=True):
    ...
    main_cnv = Canvas(master, width=x_cnv*size_cell+2*margin + gutter*(x_cnv-1), height=y_cnv*size_cell+2*margin +gutter*(y_cnv-1) , bg =COLORS['bg'])
...
if __name__ == "__main__":
    ...
    grid = create_random_grid_lc(3, 3, [0,1])
    canvas = grid_canvas(root, grid, size_cell=50, margin=20, gutter=0, show_vals=False, outline=True)
    ...
    #print(get_cell_text(canvas, 2, 0))
    #set_cell_text(canvas, 0, 0, 5)
    set_cell(canvas, grid, 2, 2, 10, 'light blue', show_vals=False, outline=False, color_text=COLORS['text_val'])
    root.mainloop()
    #print(get_grid(canvas))
